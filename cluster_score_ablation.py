...
# 设置日志
logging.basicConfig(filename='cluster_score_ablation.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
def compute_composite_score(ref_image, image, ablation=None):
    """
    计算综合打分，支持通过ablation参数进行消融实验，去掉某个打分。
    
    参数:
    - ref_image: 参考图像
    - image: 目标图像
    - ablation: 要消融的打分机制，取值为 'brightness', 'contrast', 'edge_density', 'color_histogram', 'shape_similarity'
    
    返回:
    - 综合打分
    """
    brightness_weight = 0.1 if ablation != 'brightness' else 0
    contrast_weight = 0.1 if ablation != 'contrast' else 0
    edge_density_weight = 0.1 if ablation != 'edge_density' else 0
    color_histogram_weight = 0.3 if ablation != 'color_histogram' else 0
    shape_similarity_weight = 0.4 if ablation != 'shape_similarity' else 0

    brightness_score = calculate_brightness_score(image) if ablation != 'brightness' else 0
    contrast_score = calculate_contrast_score(image) if ablation != 'contrast' else 0
    edge_density_score = calculate_edge_density(image) if ablation != 'edge_density' else 0
    color_histogram_score = calculate_color_histogram_similarity(ref_image, image) if ablation != 'color_histogram' else 0
    shape_similarity_score = calculate_shape_similarity(ref_image, image) if ablation != 'shape_similarity' else 0
...
def select_representative_frames_by_kmeans(frames, ref_frame_idx, num_clusters=5, ablation=None):
    """选择代表性帧，基于KMeans聚类，并且基于参考帧进行计算。支持消融实验去掉某个打分机制。"""
    ref_frame = frames[ref_frame_idx]
    target_size = (ref_frame.shape[1], ref_frame.shape[0])  # 宽, 高
    resized_frames = [resize_frame(frame, target_size) for frame in frames]
    ...
    for i, frame in enumerate(resized_frames):
        if i == ref_frame_idx:  # 跳过参考帧与自身的比较
            continue
        score = compute_composite_score(ref_frame, frame, ablation=ablation)
        composite_scores.append(score)
        logging.info(f"Composite score between reference frame and frame {i+1}: {score:.4f}")
...
def process_datasets_for_clustering(npz_dataset_folders, jpg_output_folder, clustered_output_folder, ref_frame_idx=0, num_clusters=5, ablation=None):
    """对所有数据集进行聚类处理，支持通过ablation参数进行消融实验。"""
    for dataset_folder in npz_dataset_folders:
        ...
        # 选择代表性帧，支持消融实验
        selected_indices, labels = select_representative_frames_by_kmeans(frames, ref_frame_idx, num_clusters, ablation=ablation)
...
clustered_output_folder = "/data/clustered_output_score_brightness"

# 进行消融实验，去掉亮度打分机制
process_datasets_for_clustering(npz_dataset_folders, jpg_output_folder, clustered_output_folder, ref_frame_idx=0, num_clusters=5, ablation='brightness')